...
public class XTMReaderTest extends AbstractXMLTestCase {
  ...
  public void testMultipleTopicMaps() throws IOException {
    ...
    assertTrue("reader doesn't recognize correct number of topic maps", tms
        .size() == 2);
    Iterator iter = tms.iterator();
    while (iter.hasNext()) {
      TopicMapIF tm = (TopicMapIF) iter.next();
      assertTrue("topic map has't got exactly two topics"
  ...
  public void testTopicName() throws IOException {
    ...
    assertTrue("wrong number of topics after import",
        tm.getTopics().size() == 6);
    assertTrue("spurious topic map constructs found", tm.getAssociations()
        .size() == 0);
  ...
  public void testVariants() throws IOException {
    ...
    assertTrue("topic has spurious children", country.getTypes().size() == 0
        && country.getRoles().size() == 0
        && country.getOccurrences().size() == 0);
    assertTrue("topic has wrong number of base names", country.getTopicNames()
        .size() == 1);

    TopicNameIF basename = (TopicNameIF) country.getTopicNames().iterator()
        .next();
    assertTrue("wrong basename value: '" + basename.getValue() + "'", basename
        .getValue().equals("Country"));
    assertTrue("wrong number of variant children", basename.getVariants()
        .size() == 1);

    VariantNameIF variant = (VariantNameIF) basename.getVariants().iterator()
        .next();
    assertTrue("wrong variant value: '" + variant.getValue() + "'", variant
        .getValue().equals("Land"));
    assertTrue("wrong scope of variant", variant.getScope().size() == 1
  ...
  public void testOccurrences() throws IOException {
    ...
    assertTrue("topic has spurious children", norway.getTypes().size() == 0
        && norway.getRoles().size() == 0 && norway.getTopicNames().size() == 1);
    assertTrue("topic has wrong number of occurrences", norway.getOccurrences()
        .size() == 4);
    ...
    while (it.hasNext()) {
      ...
      if (occ.getLocator() == null) {
        assertTrue("occurrence " + occ + " has spurious themes", occ.getScope()
            .size() == 0);
        assertTrue("occurrence has invalid resource data", occ.getValue()
            .equals("Norway is a nice country."));
      } else if (occ.getLocator().equals(norge)) {
        assertTrue("occurrence " + occ + " has spurious themes", occ.getScope()
            .size() == 0);
        assertTrue("occurrence " + occ + " has wrong type", occ.getType()
            .equals(homepage));

      } else if (occ.getLocator().equals(norwaycom)) {
        assertTrue("occurrence " + occ + " has spurious themes", occ.getScope()
            .size() == 0);
        assertTrue("occurrence " + occ + " has spurious type", occ.getType()
            .getSubjectIdentifiers().contains(PSI.getXTMOccurrence()));

      } else if (occ.getLocator().equals(visit)) {
        assertTrue("occurrence " + occ + " has wrong number of themes", occ
            .getScope().size() == 1);
        assertTrue("occurrence " + occ + " has wrong theme", occ.getScope()
  ...
  public void testTopicNameScope() throws IOException {
    ...
    assertTrue("wrong scope on basename", basename.getScope().size() == 2
  ...
  public void testTopicMapReification() throws IOException {
    ...
    assertTrue("topic map has been incorrectly merged with child topic map", tm
        .getReifier() == null);
  }
  ...
  public void testXMLBaseAndBaseLocator() throws IOException {
    ...
    assertTrue("topicmap.getBaseLocator() should not be set to xml:base", base
  ...
  public void testXTMValidIfDTDReadButDTDRefBad() throws IOException {
    ...
    try {
      ...
      fail("Invalid topic map was allowed to load"); // well, ok, it *is* valid,
                                                     // but
      // we can't know that
    } catch (OntopiaRuntimeException e) {
      assertTrue("Error not from XTM validation: " + e.getCause(),
  ...
  public void testErrorLocationReporting1() throws IOException {
    ...
    try {
      ...
      if (e.getCause() instanceof SAXParseException) {
        ...
        assertTrue("wrong error file: " + ex.getSystemId(), ex.getSystemId()
            .endsWith("invalid1.xtm"));
        assertTrue("wrong error line: " + ex.getLineNumber(), ex
            .getLineNumber() == 7);
      } else
  ...
  private void verifySingleTopicName(TopicIF topic, String name) {
    assertTrue("topic has spurious children", topic.getTypes().size() == 0
        && topic.getRoles().size() == 0 && topic.getOccurrences().size() == 0);
    assertTrue("topic has wrong number of base names", topic.getTopicNames()
        .size() == 1);
    ...
    assertTrue("basename has spurious themes", basename.getScope().size() == 0);
  }
