...
public class NameTypesPage extends AbstractTypesPage {
  ...
  public NameTypesPage(PageParameters parameters) {
    super(parameters);
  }

  @Override
  protected int getSubMenuIndex() {
    return NAME_TYPES_INDEX_IN_SUBMENU;
  }
  
  @Override
  protected Component createTreePanel(String id) {
    // create a tree
    final TreeModel treeModel = TreeModels.createNameTypesTreeModel(getTopicMapModel().getTopicMap(), isAdministrationEnabled()); 
      ...
      protected List<Component> getFunctionBoxesList(String id) {
        ...
        list.add(new CreateInstanceFunctionBoxPanel(id, getTopicMapModel()) {
          ...
          protected Topic createInstance(TopicMap topicMap, String name) {
            return topicMap.createNameType(name);
          }          
        });
        return list;
      }
