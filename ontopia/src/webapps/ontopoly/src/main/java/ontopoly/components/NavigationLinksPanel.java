...
public class NavigationLinksPanel extends Panel {
  ...
  public NavigationLinksPanel(String id, IModel model, Class linkDestination,
      IModel typeLinkLabelModel, IModel typeConfigLabelModel) {
    super(id);

    add(new BookmarkablePageLink("ontopolyLink", TopicTypesPage.class,
        new PageParameters("topicMapId="
            + ((TopicMap) model.getObject()).getId())));

    BookmarkablePageLink typeLink = new BookmarkablePageLink("typeLink",
