<!--
     ...
     ...........................................................................
     $Id: nontopoly_topic_model.jsm,v 1.1 2006/12/21 23:04:51 opland Exp $
-->
...
<module>
  ...
  <function name="heading_info">
    <tolog:set query='
      using on for i"http://psi.ontopia.net/ontology/"
        select $types from
        direct-instance-of(%topic%, $types),
        not (
         { instance-of($types, on:system-topic) |
           subject-identifier($types, "http://psi.ontopia.net/ontology/system-topic")
         })?'/>
  ...
  <function name="nav_topic_info" params="topic">
        ...
        <logic:set name="typed-names" comparator="nameComparatorWithGenerality">
          <value:intersection>
            <tm:names contextFilter="user"/>
            <value:copy of="all-typed-names"/>
          </value:intersection>
        </logic:set>
        <logic:if name="typed-names">
        <logic:then>
          <tolog:foreach query="select $name-types from
            type(%typed-names%, $name-types)
            order by $name-types?">
            <tolog:set query="type($these-names, %name-types%) order by $these-names?"/>
            <logic:set name="names">
              <value:intersection>
                <value:copy of="typed-names"/>
                <value:copy of="these-names"/>
              </value:intersection>
            </logic:set>
            <table class="shboxed" width="100%"><tr><td>
            <h3><output:element name="span"><output:attribute name="title">Names of type <tolog:out var='name-types'/></output:attribute><tolog:out var="name-types"/> (<output:count/>)</output:element></h3>
            <logic:call name="nav_name_info"/>
            </td></tr></table>
          </tolog:foreach>
        </logic:then>
...
-->
        ...
        <tolog:set query='using on for i"http://psi.ontopia.net/ontology/"
          ...
          select $tedAssocs from
           type($tedAssocs, $tedAssocType),
           instance-of($tedAssocType, on:system-topic),
           $tedAssocType /= xtm:superclass-subclass ?'/>

        <tolog:set query='using on for i"http://psi.ontopia.net/ontology/"
                          using xtm for i"http://www.topicmaps.org/xtm/1.0/core.xtm#"
          select $tedAssocTypes from
           instance-of($tedAssocTypes, on:association-type),
           $tedAssocTypes /= xtm:superclass-subclass ?'/>

        <!-- tolog query to grab all unary associations in topicAssocs -->
        <logic:set name="unaryAssocs" comparator="off">
          <tm:tolog query='using ont for i"http://psi.ontopia.net/ontology/"
                           select $A, $T from
                           association($A),
                           type($A, $T),
          ...
          <table class="shboxed" width="100%"><tr><td>
            ...
            <ul>
              <tm:associationTypeLoop name="topic" contextFilter="user"
                ...
                <logic:if name="assocs"><logic:then> <!-- there won't be one if it's been removed as a ted assoc -->
              ...
              </logic:then></logic:if><!-- not ted assoc -->
        ...
        <logic:set name="scopedAssocTypes">
          <tm:tolog select="T" query="select $T from association($myAssoc), type($myAssoc, $T), scope($myAssoc, %topic%) order by $T?"/>
        </logic:set>
  ...
  <function name="con_topic_info" params="topic">
        ...
        <tolog:set query='using on for i"http://psi.ontopia.net/ontology/"
          select $tedOccs from
           type($tedOccs, $tedOccType),
           instance-of($tedOccType, on:system-topic)?'/>
                    ...
                    <span title="Occurrence type"><output:name/></span></output:element></b>
                      ...
                      <ul>
                        <logic:foreach name="myoccs" set="myocc">
                          ...
                          <img src="images/link.png" border="0"/></output:element>
      ...
      <logic:else>
        ...
        <tolog:set query='using ont for i"http://psi.ontopia.net/ontology/"
          select $scopedOccs from
          occurrence($myTopic, $scopedOccs),
          scope($scopedOccs, %topic%),
          not({
           type($scopedOccs, ont:field-order) |
           type($scopedOccs, ont:height) |
           type($scopedOccs, ont:ted-ontology-version) |
           type($scopedOccs, ont:width) })?'/>
