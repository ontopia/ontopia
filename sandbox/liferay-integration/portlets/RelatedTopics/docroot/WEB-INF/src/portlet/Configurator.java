...
/**
 * Ths class now provides the methods to get configuration information
 * from URLs.  It also provides access to some methods of the
 * integration.
 * 
 * @author mfi
 */
public class Configurator {
   ...
   *
   * @param renderRequest The request containing information on the
   * context of the portlet and the URL
   * @return A String representing the topic id as provided in the URL
  ...
  /**
   * Parses the article id out of the URL, retrieves a topic that
   * represents this article and returns its object id.
   *
   * @param renderRequest The request containing information on the
   * context of this portlet and the URL
   * @return A String representing the topic id for the article id
   * from the URL or null if whether article id or the id of the topic
   * could not be found.
   */
  public String getTopicIdFromUrlByArticleId(RenderRequest renderRequest) {
    ...
    if(queryString != null && (queryString.lastIndexOf("article=") != -1)) {
      ...
      if(result.indexOf("&") != -1) { // there are more parameters
        result = result.substring(0, result.indexOf("&")); // the next ampersand is the delimiter for the topic id
        return OntopiaAdapter.getInstance(true).getCurrentObjectIdForArticleId(result);
  ...
  /**
   * Returns a topic id. It tries to read an article id out of the
   * config of a (presumable existent) WebContentDisplay, it then
   * tries to resolve the article id to a topic id.
   *
   * @param renderRequest The request containing information on the
   * context of this portlet (like the layout in which it is viewed)
   * @return A String representing the object id of a topic, or null
   * if no WebContentDisplay could be found or no object id could be
   * resolved
   */
  public String findTopicIdFromNextWCD(RenderRequest renderRequest) {
    ...
    try{
      // this brings us the first WebContentDisplay Portlet
      // (identified by 56_INSTANCE_") on that page. Returns
      // instance-id.
      ...
      try{
        // These objects contain useful information like the article
        // id as a string
        List pref = PortletPreferencesLocalServiceUtil.getPortletPreferences(layout.getPlid(), portletId);
        // FIXME: hardwire the first hit is maybe not a good idea for
        // the future.
        ...
        // not sure how to handle this SystemException so throwing
        // it at runtime
        throw new OntopiaRuntimeException(se);
      }
    } catch(StringIndexOutOfBoundsException siobe) {
      // this is expected and not too unusual really. No
      // WebContentDisplay there, to indicate the failure to produce
      // useful information we return null.
      return null;
    }
  }

  /**
   * Gets a read-only TopicMapIF object from the Ontopia integration.
   *
   * @return a TopicMapIF object
   */
  public TopicMapIF getTopicmap() {
    return OntopiaAdapter.getInstance(true).getTopicMap();
  }
